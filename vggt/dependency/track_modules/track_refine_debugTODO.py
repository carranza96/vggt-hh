...
def refine_track(
    ...
    if chunk < 0:
        ...
        for p in torch.split(patches, chunk):
            chunk_result = fine_fnet(p)
            patch_feat_list.append(chunk_result)
            # Delete the chunk immediately after processing
            del p, chunk_result
        ...
        for tensor in patch_feat_list:
            del tensor
        patch_feat_list.clear()
        del patch_feat_list
        torch.cuda.empty_cache()
    ...
    # Store original patch_feat before reshaping to delete it
    patch_feat_original = patch_feat
    patch_feat = patch_feat.reshape(B, S, N, C_out, psize, psize)
    # Delete the original [136656, 32, 31, 31] tensor
    del patch_feat_original
    torch.cuda.empty_cache()
    ...
    del patch_feat, patch_query_points
    torch.cuda.empty_cache()
